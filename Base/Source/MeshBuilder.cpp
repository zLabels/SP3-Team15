...
*/
/******************************************************************************/
Mesh* MeshBuilder::GenerateCrossHair(const std::string &meshName, float colour_r, float colour_g,
	float colour_b, float length)
{
...
Mesh* MeshBuilder::GenerateTerrain(const std::string &meshName, const std::string &file_path, std::vector<unsigned char> &heightMap)
{
	...
	{   
		...
		{ 
			...
			v.normal = calculateNormal(static_cast<float>(x),static_cast<float>(z),heightMap);
...
Mesh* MeshBuilder::Generate2DHook(const std::string &meshName, Color color,int pos_x, int pos_y, int width, int height)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
	// Vertex #18 
	v.pos.Set(-0.5f * width, -0.5f * height, 0);
	v.color = color;
	v.normal.Set(0, 0, 1);
	v.texCoord.Set(0, 0);
	vertex_buffer_data.push_back(v);
...
CSpriteAnimation* MeshBuilder::GenerateSpriteAnimation(const std::string &meshName, unsigned numRow, unsigned numCol)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
	...
	CSpriteAnimation *mesh = new CSpriteAnimation(meshName, numRow, numCol);
...
Mesh* MeshBuilder::GenerateTile(const std::string	&meshName, unsigned numRow, unsigned numCol,unsigned selectedRow,unsigned selectedCol,unsigned tileSize)
{
	Vertex v;
	std::vector<Vertex> vertex_buffer_data;
	std::vector<GLuint> index_buffer_data;
	...
	float u1 = selectedCol * width;
	float v1 = 1.f - height - selectedRow * height;
	v.pos.Set(0* tileSize, 0* tileSize, 0);
	v.texCoord.Set(u1, v1);
	vertex_buffer_data.push_back(v);

	v.pos.Set(1* tileSize, 0* tileSize, 0);
	v.texCoord.Set(u1 + width, v1);
	vertex_buffer_data.push_back(v);

	v.pos.Set(1* tileSize, 1* tileSize, 0);
	v.texCoord.Set(u1 + width, v1 + height);
	vertex_buffer_data.push_back(v);

	v.pos.Set(0* tileSize, 1* tileSize, 0);
	v.texCoord.Set(u1, v1 + height);
	vertex_buffer_data.push_back(v);
